...
dt <- count18plus %>% 
  ...
  left_join(count18plus5conds %>% 
              ...
              mutate(dep_type=ifelse(type %in% c("IMD", "income"), type, NA)) %>% 
              fill(dep_type, .direction="down") %>% 
              filter(!is.na(counts5conds)), by = c("quintile" = "type", "sex", "age_grp", "dep_type")) %>% 
...
plus18_5cond<-dep_extract(plus18_5cond, cond4=TRUE)
plus65_5cond<-dep_extract(plus165_5cond, cond4=TRUE)
